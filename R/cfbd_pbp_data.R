...
#' @description 
#' \describe{
#' \item{`add_play_counts()`: Adds play counts to Play-by-Play data pulled from the API's raw game data}
#' \item{`add_yardage()`: Add yardage extracted from play text}
#' \item{`add_player_cols()`: Add player columns extracted from play text}
#' \item{`clean_drive_dat()`: Create new Drive results and id data}
#' \item{`clean_pbp_dat()`:  Clean Play-by-Play data}
#' \item{`penalty_detection()`: Adds penalty columns to Play-by-Play data pulled from the API}
#' \item{`prep_epa_df_after()`: Creates the post-play inputs for the Expected Points model to predict on for each game}
#' \item{`clean_drive_info()`: Cleans CFB (D-I) Drive-By-Drive Data to create `pts_drive` column}
#' }
