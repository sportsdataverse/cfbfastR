...
#'
#' @param clean_pbp_dat (\emph{data.frame} required): Clean PBP as input from \code{\link[cfbfastR:cfbd_pbp_data]{cfbfastR::cfbd_pbp_data()}})
#' @param ep_model (\emph{model} default cfbfastR:::ep_model): Expected Points (EP) Model
#' @param fg_model (\emph{model} default cfbfastR:::fg_model): Field Goal (FG) Model
...
#'
#' @param df (\emph{data.frame} required): Clean Play-By-Play data.frame as can be pulled from `cfbd_pbp_dat()`
#' @param current_probs (\emph{data.frame} required): Expected Points (EP) model raw probability outputs from initial prediction
#' @param ep_model (\emph{model}, default `cfbfastR:::ep_model`): FG Model to be used for prediction on field goal (FG) attempts in Play-by-Play data.frame
#' @param fg_mod (\emph{model}, default `cfbfastR:::fg_model`): FG Model to be used for prediction on field goal (FG) attempts in Play-by-Play data.frame
#'
#' @keywords internal
