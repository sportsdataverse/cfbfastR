...
test_that("ESPN CFB Team Record", {
  skip_on_cran()

  cols <- c(
    "season", "season_type", "team_id", "record_type", "record_name",
    "record_summary", "stat_name", "stat_value", "stat_display"
  )
  ...
  x <- espn_cfb_team_record(team_id = 61, year = 2024)

  y <- espn_cfb_team_record(team_id = 61, year = 2023)

  if (is.null(x) || !is.data.frame(x) || nrow(x) == 0) {
    skip("No ESPN team record data returned at test time")
  }
  ...
  z <- espn_cfb_team_record(team_id = 61, year = 2024, team_detail = FALSE)
