...
test_that("ESPN CFB Team Events", {
  skip_on_cran()
  ...
  x <- espn_cfb_team_events(team_id = 61, year = 2024)

  y <- espn_cfb_team_events(team_id = 61, year = 2023)

  if (is.null(x) || !is.data.frame(x) || nrow(x) == 0) {
    skip("No ESPN team events data returned at test time")
  }
  ...
  z <- espn_cfb_team_events(team_id = 61, year = 2024, team_detail = FALSE)
