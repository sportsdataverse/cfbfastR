...
test_that("ESPN CFB Team Ranks", {
  skip_on_cran()

  cols <- c(
    "season", "team_id", "week", "poll_id", "poll_name", "poll_short_name",
    "poll_type", "headline", "date", "current_rank", "previous_rank",
    "trend", "points", "first_place_votes", "record_summary"
  )
  ...
  x <- espn_cfb_team_ranks(team_id = 61, year = 2024)

  y <- espn_cfb_team_ranks(team_id = 61, year = 2023)

  if (is.null(x) || !is.data.frame(x) || nrow(x) == 0) {
    skip("No ESPN team ranks data returned at test time")
  }
  ...
  z <- espn_cfb_team_ranks(team_id = 61, year = 2024, team_detail = FALSE)
