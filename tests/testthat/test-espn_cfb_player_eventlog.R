...
test_that("ESPN CFB Player Event Log", {
  skip_on_cran()

  cols <- c(
    "athlete_id", "season", "game_id", "team_id", "played",
    "event_ref", "competition_ref", "statistics_ref"
  )
  ...
  x <- espn_cfb_player_eventlog(athlete_id = 102597, year = 2024)

  y <- espn_cfb_player_eventlog(athlete_id = 102597, year = 2023)

  if (is.null(x) || !is.data.frame(x) || nrow(x) == 0) {
    skip("No ESPN player event log data returned at test time")
  }
  ...
  z <- espn_cfb_player_eventlog(athlete_id = 102597, year = 2024,
                                team_detail = FALSE,
