...
`cfbfastR` is the R package for clean, tidy college-football play-by-play, schedule,
roster, ratings, betting, recruiting, and box-score data. It wraps **CollegeFootballData
(CFBD)**, **ESPN** college-football endpoints, plus **Fox Sports** and **Yahoo**.
Companions: `sportsdataverse`/`cfbfastR-py` (Python CFB surface), and the
...
When this guide differs from the repo, treat `CONTRIBUTING.md` and the current
`tests/testthat/` suite as authoritative.
...
CI (`.github/workflows/`): `R-CMD-check.yaml` (windows + ubuntu release/oldrel-1, weekly
cron, sets `CFBD_API_KEY` secret), `pkgdown.yaml` (push to main + release), `rhub.yaml`
(manual dispatch). Markdown TOCs are regenerated by `Rscript tools/run_doctoc.R --maxlevel 2 NEWS.md CLAUDE.md ...`
(no-Node doctoc replacement, idempotent). After DESCRIPTION edits run `usethis::use_tidy_description()`.
...
`sportsdataverse-data` release tag `cfbfastR_cfb_pbp` (**floor 2014**, default
`most_recent_cfb_season()`); `load_cfb.R` has `load_cfb_rosters/_schedules/_teams`.
...
(`pbp_adapters.R`) → engine → play taxonomy centralized in `pbp_taxonomy.R` (single source
of truth, replaced 4 drifted copies) → `.pbp_apply_output_schema(df, output=)` applies
`.pbp_output_order` column order + the **`"default"`/`"lean"`/`"full"` tier** selectors
(`pbp_output_schema.R`). Models are mgcv/nnet/xgboost EP/FG/WP. ESPN team/venue/coach
metadata the bare payload omits is resolved by `.espn_pbp_game_meta()`.

## Conventions

- **Native pipe `|>` only** — `%>%`/`magrittr` swept out; not in Imports. `|>` rejects `[[`
  as RHS (use `purrr::pluck()`) and has no `.` placeholder (use `_` or `\(x)`).
- **Initialize every `tryCatch` return variable BEFORE the block** (e.g. `plays_df <- data.frame()`).
  On API error the handler runs, the var never binds, and `return(var)` throws
  `object not found` instead of the intended `cli_alert_danger()` + empty fallback. Applies
  to all return names; init `list()`/`NULL`/`data.frame()` as appropriate.
- **Scalar-default args, not `c(...)` + `match.arg()`.** Default `output = "default"`,
  document choices in `@param`, validate with `cli::cli_abort()`. The c-vector form silently
  picks the first element on a vector arg.
- **`cli` for all messaging:** `cli_alert_danger/warning/info/success` inside handlers;
  `cli_abort/cli_warn` for raised conditions. Pass `{conditionMessage(e)}`, never raw `{e}`
  (glue coerces the condition).
- **Pipeline tail:** `... |> janitor::clean_names() |> make_cfbfastR_data("<desc>", Sys.time())`
  sets class `c("cfbfastR_data","tbl_df","tbl","data.table","data.frame")` + timestamp/type
  attrs. Apply it **after** the final dplyr verb (dplyr strips the class).
- **Column-drift resilience:** drop with `dplyr::select(-dplyr::any_of("col"))`, rename with
  `rename(any_of(c(new = "old")))` — both APIs add/rename columns over time.
- **Null safety:** `%||%` (re-exported from rlang via `@importFrom`; don't redefine).
- **ESPN self-describing returns:** catalog wrappers call `.attach_query_meta_auto()`
  (`utils_attach_query_meta.R`), which introspects caller formals and lifts query args
  (`season`/`week`/`team_id`/...) onto the frame as leading columns (`year`→`season`); add a
  conventionally-named arg and it auto-flows. Response columns win on name collision.
- **Proxy:** `get_req()` resolves `proxy=` arg → `getOption("cfbfastR.proxy")` →
  `http(s)_proxy` env. Thread `proxy = NULL`/`...` through new wrappers for per-call override.

## Caching

ESPN catalog calls are memoised (`cachem` + `memoise`, wired in `zzz.R` `.onLoad`):
`options(cfbfastR.cache = "memory"|"filesystem"|"off")` (default `memory`),
...
- Tests gate network calls with `skip_on_cran()` + `skip_on_ci()` (and a few `skip_if()`).
  `setup-cfbd-throttle.R` hot-swaps `get_req()` to `Sys.sleep()` between CFBD calls (avoids
  429 storms); tune with `options(cfbfastR.test_request_delay=)`. `Config/testthat/edition: 3`.
- **Column assertions use the subset direction:** `expect_in(sort(expected), sort(colnames(x)))`
  (expected ⊆ actual) so a new upstream column doesn't fail the run. Add a skip-if-empty guard
...
- **ESPN `$ref` payloads:** core API returns `{"$ref": "..."}` placeholders instead of inlined
  team/athlete/venue records. Route team-metadata consumers through `.espn_pbp_game_meta()`
  (falls back to cached `.espn_cfb_team_lookup()`), not raw field reads.
- **`season` convention:** calendar year of the fall season (Jan-2024 Rose Bowl is `season=2023`);
  OT possessions model as drives with `period >= 5`.
- Functions deprecated via `lifecycle::deprecate_stop()` + `@keywords internal` live in
